...
function buildEvidenceChain({ traceId, userMessage, assistantReply, memory }) {
  const memoryType = chatMemory.normalizeMemoryType(memory?.memoryType || 'fakta');
  const memoryKey = chatMemory.normalizeMemoryKey(memory?.key || 'memori');
  const memoryValue = String(memory?.value || '').trim();
  const memoryCategory = String(memory?.category || 'umum').trim().toLowerCase() || 'umum';

  return [
    {
      step: 'user_message',
      confidence: 0.93,
      source: compactHistoryMessage(userMessage, 220)
    },
    {
      step: 'assistant_reply',
      confidence: 0.84,
      source: compactHistoryMessage(assistantReply, 220)
    },
    {
      step: 'memory_candidate',
      confidence: 0.88,
      source: compactHistoryMessage(`${memoryKey}|${memoryType}|${memoryCategory}|${memoryValue}`, 220)
    },
    {
      step: 'trace_id',
      confidence: 1,
      source: String(traceId || '')
    }
  ];
}

function mergeLockStatus(currentStatus = 'saved', nextStatus = 'saved') {
  if (nextStatus === 'draft') return 'draft';
  if (nextStatus === 'locked' && currentStatus !== 'draft') return 'locked';
  if (currentStatus === 'draft' || currentStatus === 'locked') return currentStatus;
...
export default async function handler(req, res) {
  // ─── PHASE 0: Trace ID Injection ───
  const traceId = traceGuard.injectTraceId(req);
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      traceGuard.logAuditTrail(supabase, traceId, '/api/chat', 'GET', 500, { error: err.message });
      return res.status(500).json({ error: err.message });
    }
  }

  if (req.method !== 'POST') {
    traceGuard.logAuditTrail(supabase, traceId, '/api/chat', req.method, 405, {});
...
- Tag [MEMORY:...] dan [MEMORY_FORGET:...] adalah instruksi sistem internal, JANGAN tampilkan ke user, letakkan di paling akhir respons.`
    ...
    const ambiguityPayload = REASONING_PREVIEW_ENABLED
      ? chatPreview.analyzeAmbiguityPreview(userMessage, person, allPersons || [])
      : null;
    const emotionAnalysis = REASONING_PREVIEW_ENABLED
      ? chatPreview.detectConversationEmotion(userMessage, recentHistory)
      : null;
    const previewPayload = REASONING_PREVIEW_ENABLED
      ? {
          preview_version: 2,
      ...
      : null;
    const clientPreviewPayload = REASONING_PREVIEW_ENABLED
      ? chatPreview.buildClientPreviewPayload(previewPayload)
      : null;
...
`);
    ...
    if (REASONING_PREVIEW_ENABLED && clientPreviewPayload) {
      res.write(`data: ${JSON.stringify({
...
- Ringkasan checkpoint harus berdiri sendiri, menyatukan konteks lama + konteks baru, maksimum 2200 karakter dan tetap padat.`
        ...
        for (const mem of detectedMemories) {
          try {
            ...
            if (guardResult.status === 'draft' || guardResult.status === 'locked') {
              ...
              console.log(`[LockGuard] Skip overwrite for "${normalizedKey}" with status=${guardResult.status}`);
              continue;
            }
    ...
    traceGuard.logAuditTrail(supabase, traceId, '/api/chat', 'POST', 500, { error: error.message, streaming: true, lock_status: 'locked' });
    res.write(`data: ${JSON.stringify({ error: error.message, trace_id: traceId, lock_status: 'locked' })}\n\n`);
    res.end();
  }
}
